...
class Cl_User
{
	...
	public function login( array $data )
	{
		...
		if( !empty( $data ) ){
			...
			$password = md5( $password );
			$query = "SELECT user_id, name, email, created FROM user where email = '$email' and password = '$password' ";
	...
	public function google_login( array $data )
	{
		...
		if( $count == 1){
			...
			$password = $this->randomPassword();
			$password1 = md5( $password );			
			$query = "INSERT INTO user (user_id, password, name, email, social_id, picture, created) VALUES (NULL, '$password1', '$name', '$email', '$social_id', '$picture', CURRENT_TIMESTAMP)";
